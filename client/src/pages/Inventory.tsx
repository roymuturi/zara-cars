...
export default function Inventory() {
  ...
  return <PageFrame><section className="page-hero inventory-hero"><div><p className="section-kicker">Zara Cars / Live stock</p><h1>Find the one<br /><span>that fits.</span></h1><p>Every unit below has a make, model, price, location, and real pipeline status. Filter it down, open the file, or send the stock number to a human.</p></div><div className="page-hero-aside"><Link href="/dealer" className="text-link">Dealer workspace <ArrowRight size={15} /></Link></div></section>
    <section className="inventory-workspace"><div className="make-rail"><div><p className="section-kicker">Browse by company</p><h2>Start with a make.</h2></div><button className={`make-rail-all ${make === "All makes" ? "active" : ""}`} onClick={() => chooseMake("All makes")}>All makes</button><div className="make-logo-track"><div ref={makeScrollRef} className="make-logo-scroll" role="region" aria-label="Browse vehicle makes" onPointerDown={beginMakeRailDrag} onPointerMove={moveMakeRailDrag} onPointerUp={endMakeRailDrag} onPointerCancel={endMakeRailDrag}>{makeOptions.map(item => <button key={item} className={`make-logo-button ${make === item ? "active" : ""}`} onClick={() => chooseMake(item)} aria-label={`Filter by ${item}`}><MakeBadge make={item} /></button>)}</div><div className="make-rail-dots" aria-hidden="true">{[0, 1, 2].map(index => <span key={index} className={makeRailPage === index ? "active" : ""} />)}</div><p className="make-rail-hint"><MoveHorizontal size={13} /> Swipe for more</p></div></div>

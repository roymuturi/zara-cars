...
export default function ServicePage({ type }: { type: ServiceType }) {
  ...
  return <PageFrame><section className="service-hero"><div><p className="section-kicker">{page.eyebrow}</p><h1>{page.title.split("\n").map((line, index) => <span key={line} className={index === 1 ? "red-line" : ""}>{line}<br /></span>)}</h1><p>{page.intro}</p><div className="hero-actions">{type === "finance" ? <button type="button" className="button button-red" onClick={() => scrollToId("finance-calculator")}>{page.actions[0]} <ArrowRight size={16} /></button> : type === "trade" ? <button type="button" className="button button-red" onClick={() => scrollToId("trade-in-form")}>{page.actions[0]} <ArrowRight size={16} /></button> : type === "contact" ? <button type="button" className="button button-red" onClick={() => scrollToId("contact-form")}>{page.actions[0]} <ArrowRight size={16} /></button> : <Link href="/inventory" className="button button-red">{page.actions[0]} <ArrowRight size={16} /></Link>}</div></div><div className="service-orbit-card"><span className="service-icon"><Icon size={24} /></span><small>Designed around the next step</small><strong>{page.actions[1]}</strong><p>Clear, local, and ready for a real conversation.</p><div className="service-checks"><span><Check size={16} /> Kenyan context</span><span><Check size={16} /> No pressure language</span><span><Check size={16} /> Human follow-up</span></div></div></section><section className="service-grid"><div><p className="section-kicker">How it works</p><h2>Small steps.<br />Fewer unknowns.</h2></div><div className="service-steps"><div><span>01</span><div><strong>{page.actions[0]}</strong><p>Start with what you know, then let the team fill in the gaps without making you repeat yourself.</p></div><CarFront size={19} /></div><div><span>02</span><div><strong>{page.actions[1]}</strong><p>Get a practical next step based on the actual unit, location, status, and paperwork.</p></div><FileCheck2 size={19} /></div><div><span>03</span><div><strong>{page.actions[2]}</strong><p>Move forward only when the details feel right. A viewing, a call, or a WhatsApp answer is enough.</p></div><CalendarDays size={19} /></div></div></section>{type === "finance" && <FinanceCalculator />}{type === "trade" && <TradeInForm />}{type === "contact" && <ContactForm />}<section className="service-cta"><div><p className="section-kicker">Ready when you are</p><h2>Good cars deserve<br />clear decisions.</h2></div><Link href="/inventory" className="button button-red">See live stock <ArrowRight size={16} /></Link>{(type === "finance" || type === "trade") && <WhatsAppButton />}</section></PageFrame>;
}
